...
public class GeradorArquivosOrdenados{
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            System.out.println("Uso: java GeradorArquivosOrdenados <quantidade> <arquivo_saida>");
            System.exit(1);
        }
    ...
    private static void gerarArquivo(int quantidade, String arquivoSaida) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(arquivoSaida, StandardCharsets.UTF_8))) {
            for (int i = 1; i <= quantidade; i++) {
                ...
                int nota = random.nextInt(11); // Gera uma nota inteira entre 0 e 10
                writer.write(i + ";" + nome + ";" + nota + "\n");
                if (i % 100_000 == 0) {
                    System.out.println(i + " registros ordenados gerados...");
                }
            }
